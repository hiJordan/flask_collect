...
def path_with_query(path, query):
    ...
    q_name = q[0]
    q_height = q[1]
    cons_path = path + '?{}={}&{}={}'.format(q_name[0], q_name[1], q_height[0], q_height[1])
    return cons_path
...
def test_path_with_query():
    ...
    log('字典拼接url:   ', path_with_query(path, query))
    assert path_with_query(path, query) in expected, '路径不在预期中'
...
def header_from_dict(headers):
    ...
    'Content-Type: text/html\r\nContent-Length: 127\r\n'
    """
    header_split = tuple(headers.items())
    str_header = ''
    for line in header_split:
        str_line = str(line[0]) + ':' + str(line[1])
        str_header += str_line + '\\r\\n'
    return str_header
...
def test_header_from_dict():
    ...
    log('字典拼接str:  ', header_from_dict(response_header))
